...
- `../aicraft-spec/references/feature-contract.md` for feature identity and readiness;
- `../aicraft-spec/references/quick.md` for a quick target;
- root `AGENTS.md`;
- `../aicraft-tune/references/config.md`, the committed `.aicraft/config.yaml`, and any `.aicraft/config.local.yaml` overlay, judging each configured value a mode check depends on, such as tracker mode, by that contract's resolved configuration; and
- the cited `spec.md`, `design.md`, task plan, applicable accepted ADRs, or quick file.

For a retrospective report under `docs/.retro/`, read [`../aicraft-temper/references/report-format.md`](../aicraft-temper/references/report-format.md) for the required sections, incident record, and evidence grades, and [`../aicraft-temper/references/process.md`](../aicraft-temper/references/process.md) for the analysis discipline, routing tiers, and operator escalation, together with root `AGENTS.md`. Those two references are the report's governing contract; the feature and quick references above do not govern it. The corpus the report cites is evidence to verify, not an authority.

Accepted ADRs outrank `specs/`, which outrank derived `docs/`. Report unresolved conflicts. A missing authority blocks review; missing configuration blocks only its dependent checks. Never reconstruct intent or invoke `aicraft-tune`. Resolve every cited path inside the repository.
...
Task-plan findings resolve in `aicraft-draft`. Feature and quick-artifact findings resolve in `aicraft-spec`. Retrospective-report findings resolve in `aicraft-temper`, which edits the same report in place. Code findings resolve in `aicraft-forge` unless the finding changes product scope, acceptance behavior, or the passed plan's approach; those findings resolve in Draft. The resolution column names the owner but Review invokes no producer skill.
