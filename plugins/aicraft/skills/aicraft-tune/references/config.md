...
AiCraft configuration lives in two files under `.aicraft/`:

- `.aicraft/config.yaml` — the committed base. It is tracked in git, shared by every collaborator, and standalone-valid: it holds the complete configuration and passes validation on its own.
- `.aicraft/config.local.yaml` — the optional user-local overlay. It is gitignored, never committed, and may set only user-scoped keys.
...
Every mapping in either file is closed: reject unknown keys at every level. Reject duplicate YAML keys, non-string mapping keys, and values of the wrong YAML type. Strings required below must be nonempty after trimming. The base records the plugin version the repository was last migrated to in `aicraft_version`; no value has a read-time default.

## Scope split

Repo-scoped settings live only in the base: `aicraft_version`, the whole `tracker` section, `foreman.default_branch`, `foreman.release_branch`, `foreman.clarification_assignees`, and `pull_requests.merge_strategy`. A repo-scoped key in the overlay is a validation error.
...
The base takes this shape and the root requires exactly `aicraft_version`, `tracker`, `foreman`, and `pull_requests`:

```yaml
aicraft_version: "0.8.1"
...
`aicraft_version` is a string of three dot-separated non-negative integers: the plugin version this repository was last migrated to. The current plugin version is the newest `## vX.Y.Z` heading in [`migrations.md`](migrations.md). Compare the two component by component as integers.
...
**Layer question.** When a reconfiguration changes only user-scoped keys, ask once where the change applies: `For everyone (Recommended)` — write it into the committed `.aicraft/config.yaml`; `Just for you` — write it into the gitignored `.aicraft/config.local.yaml` overlay. A change that touches any repo-scoped key writes to the base and never offers the overlay. An overlay write replaces its whitelisted keys atomically, with `backend` and `herdr` written as a unit.
