...
- `aicraft-cast` creates or retrofits only the project frame and clean baseline.
- `aicraft-spec` creates or resumes every feature triplet, including the first MVP feature, and every quick-task file. It owns authorized GitHub projection.
- `aicraft-draft` writes and revises task plans.
- `aicraft-forge` implements one passed task plan and prepares the task PR.
- `aicraft-review` independently reviews exact plan or code targets.
- `aicraft-land` owns final-head checks, landing, and tracker closure.
- `aicraft-temper` produces retrospectives.
- `aicraft-foreman` coordinates the delivery loop without performing those stages itself.

## Task tracker authority

| Data | Authority | Rule |
| --- | --- | --- |
| Project tracker and target repository | `.aicraft/config.yaml` | Require one complete valid tracker section. Delegate missing or invalid configuration to `aicraft-tune` before inspecting GitHub. |
...
Read the resolved AiCraft configuration — the committed `.aicraft/config.yaml` plus any gitignored `.aicraft/config.local.yaml` overlay — before AiCraft task work. Invoke `aicraft-tune` when required configuration is missing or invalid, except under a report-only contract: `aicraft-review` reports invalid configuration as a finding and never invokes Tune.
...
Do not create or reconcile generated feature and task issues by hand. Route projection or mapping drift to `aicraft-spec`.
