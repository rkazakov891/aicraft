...
description: >
  ...
  structure, or committing AiCraft's base configuration with the frame. Cast commits one clean
...
Create the project frame that makes later SDD artifacts durable. Cast produces no feature contract. `aicraft-spec` is the sole producer of every feature triplet, including the first MVP feature, and every quick-task file.
...
- `../aicraft-tune/references/config.md` for the complete closed configuration contract;
...
2. **Resolve configuration.** Read `../aicraft-tune/references/config.md`, then the resolved configuration: the committed `.aicraft/config.yaml` base plus any `.aicraft/config.local.yaml` overlay. When the resolved configuration is missing or invalid, or the request changes a configuration value, invoke `aicraft-tune` before gathering scaffold facts and resume only after `Status: done`. Apply the Tune round-trip rules in `references/scaffold.md`: they own valid-value authority, the stale-ignore repair, non-done Tune routing, and the base-file rollback boundary. A Tune question this run cannot answer ends the run with `configuration_required`; any other non-done Tune result Cast does not own ends it with `configuration_unresolved`, quoting Tune's terminal line in the detail.
