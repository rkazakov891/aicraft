...
Read the root `AGENTS.md` for project instructions. Resolve tracker and runtime settings from the resolved configuration — the committed `.aicraft/config.yaml` plus any `.aicraft/config.local.yaml` overlay — with [`references/configuration.md`](references/configuration.md). If the configuration is missing or invalid, invoke `aicraft-tune` for the affected section and resume only after `Status: done`. Never infer or default a missing value while reading configuration.
...
Load [`references/review-round.md`](references/review-round.md) only after material findings. Confirm `.aicraft/foreman/` is ignored by the `/.aicraft/*` rule. Resume a verified active checkpoint automatically. Without one, apply `loop.md`'s takeover bootstrap before creating a run directory from [`templates/run-dir.md`](templates/run-dir.md).
...
- `specifier-<identity>` runs `aicraft-spec` only to resume an existing contract, resolve Spec findings, or complete its prepared outward handoff.
- `spec-reviewer-<identity>` runs `aicraft-review` in plan mode on the feature triplet or quick file.
- `drafter-<identity>` runs `aicraft-draft`.
- `plan-reviewer-<identity>` runs `aicraft-review` in plan mode.
- `forge-<identity>` runs `aicraft-forge` and owns code-finding remediation.
- `code-reviewer-<identity>` runs `aicraft-review` in code mode.
- `land-<identity>` runs `aicraft-land` for finalization or external completion.
- `review-evidence-<identity>` runs `aicraft-review` in evidence mode.
- `temper-<feature>` runs `aicraft-temper` once after confirmed feature closure.
- `retro-reviewer-<feature>` runs `aicraft-review` in plan mode on the local retrospective report.
