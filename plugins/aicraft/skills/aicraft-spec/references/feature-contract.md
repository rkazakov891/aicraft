...
- a readable root `AGENTS.md` and a valid resolved configuration whose committed `.aicraft/config.yaml` sets `tracker.mode: local` or `tracker.mode: github`;
...
Read tracker mode and target from the committed `.aicraft/config.yaml` before inspecting remotes or GitHub. Local mode performs no output-tracker discovery or readiness check. A producer skill delegates a missing or invalid tracker section to Tune and resumes only after `Status: done`; report-only Review instead reports it as blocking.

A local mapping is exactly `spec_issue: null`. A GitHub mapping is `spec_issue: TBD` or one positive issue number. Reject a `tracker` or `spec_issue` field in `tasks.md`. Tune owns tracker mode and target in the committed `.aicraft/config.yaml`; `spec.md` owns the feature-issue mapping; `tasks.md` owns only T-ID-to-task-issue mappings.
