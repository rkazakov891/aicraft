...
git init -q -b main
git config user.name "AiCraft Eval"
git config user.email "eval@example.test"
git add .gitignore .aicraft/config.yaml AGENTS.md bin docs prepare-eval.sh specs src tests
...
cat > .aicraft/foreman/plan-review.md <<EOF
## Review report
...
if [ "$mode" = implemented ]; then
  ...
  cat > .aicraft/foreman/forge-assignment.md <<EOF
Producer: aicraft-forge
...
Plan review: .aicraft/foreman/plan-review.md covering $plan_sha
EOF
fi
