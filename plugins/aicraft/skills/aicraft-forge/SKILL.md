...
Read root `AGENTS.md` for project instructions. Read `../aicraft-tune/references/config.md` completely, then validate the resolved configuration — the committed `.aicraft/config.yaml` plus any `.aicraft/config.local.yaml` overlay — against that contract. If the configuration is missing or invalid, invoke `aicraft-tune` for the affected section and resume only after `Status: done`. A Tune question this run cannot answer ends the run with `configuration_required`; any other non-done Tune result ends it with `configuration_unresolved`, quoting Tune's terminal line in the detail. Read `../aicraft-spec/references/feature-contract.md` for feature identity and readiness. For a quick task, also read `../aicraft-spec/references/quick.md`.
...
- A free-form fix must already be one coherent, fully specified change. Route larger or underspecified product work to `aicraft-spec`.

Gate every task before implementation. Require feature or quick readiness and completed dependencies. Feature dependencies use `tasks.md` in local mode and the tracker in hosted mode. Quick dependencies always use their referenced quick-file checkbox. Route an unready feature or quick artifact to `aicraft-spec`; Forge never completes that artifact.
...
- one committed semantic task plan produced by `aicraft-draft`;
...
A missing plan, missing pass, stale verdict, or plan-path or commit mismatch changes no source. Return the exact task and current plan evidence to `aicraft-draft` with `draft_required`. Never create, rename, or revise the plan.
...
A finding that changes product scope, acceptance behavior, or the plan's declared approach is not code remediation. Return the finding, exact plan path, and plan commit to `aicraft-draft` with `draft_required`. Do not rewrite the approved plan.
